...
pipeline = RAGOpenAiPipeline(vector_storage_kwargs={'chunk_size': 800, 'chunk_overlap': 200}, openai_system_prompt=old_prompt)
...
async def start_handler(message: Message) -> None:
    ...
    welcome_text = (
        "👋 <b>Добро пожаловать в LegalAI бота!</b>\n\n"
        "Я могу отвечать на ваши вопросы на основе ваших юридических документов.\n\n"
        "Для начала работы отправьте мне ваш <b>уникальный токен</b> с помощью команды /token,\n"
...
async def help_handler(message: Message) -> None:
    ...
    help_text = (
        ...
        "<code>/token ваш_уникальный_токен</code>\n\n"
        "2. Задавайте вопросы по вашим документам\n\n"
        "Пример:\n"
        "<code>/token мой_уникальный_токен_123</code>\n"
        "Затем:\n"
        "<code>Какие документы нужны для расторжения договора?</code>"
    )
...
async def token_handler(message: Message) -> None:
    ...
    await message.answer(
        f"🔑 Токен установлен: <code>{token}</code>\n\n"
...
async def message_handler(message: Message) -> None:
    ...
    if user_id not in user_tokens:
        await message.answer(
            ...
            "Пример: <code>/token ваш_уникальный_токен</code>"
        )
        return
