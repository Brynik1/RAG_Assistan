...
async def help_handler(message: Message, user_states) -> None:
    """Обработчик команды /help - вывод информации о доступных командах"""
    ...
    help_text = (
        "🤖 <b>Онбординг-бот для сотрудников</b>\n\n"
        "Этот бот помогает новым сотрудникам знакомиться с документами компании.\n\n"
    ...
    # Добавляем инструкцию по вопросам
    help_text += (
        "\n💡 Вы можете задавать вопросы по содержанию ваших документов в свободной форме.\n"
        "Примеры:\n"
        "   • Каков график работы?\n"
...
async def documents_handler(message: Message, user_states, pipeline) -> None:
    ...
    await message.answer(
        f"📂 Вам доступны следующие документы для токена: `{token}`",
        parse_mode=ParseMode.MARKDOWN
    )
