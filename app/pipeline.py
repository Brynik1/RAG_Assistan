...
class RAGOpenAiPipeline:
    ...
    **Важно:** Все выводы должны однозначно следовать из предоставленных материалов без домыслов.
    ...
    Преобразуй запрос в ключевые теги/фразы для поиска.
    Удали приветствия, вежливые формы и избыточные формулировки.
    Исправь ошибки в написании.
    Верни только суть запроса через запятую.

    Примеры:
    1. Вход: "Здравствуйте, какие документы нужны для отпуска?"
       Выход: "документы для отпуска"
    2. Вход: "Спасибо! Сколько дней отпуска за первый год?"
